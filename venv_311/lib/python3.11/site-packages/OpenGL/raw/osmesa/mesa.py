...
def _f(function):
    return _p.createFunction(function, _p.PLATFORM.OSMesa, None, error_checker=None)
...
def OSMesaGetDepthBuffer(c):
    ...
    if _p.PLATFORM.GL.OSMesaGetDepthBuffer(
        c, ctypes.byref(width), ctypes.byref(height), ctypes.byref(bytesPerValue), ctypes.byref(buffer)
    ):
...
def OSMesaGetColorBuffer(c):
    ...
    if _p.PLATFORM.GL.OSMesaGetColorBuffer(
        c, ctypes.byref(width), ctypes.byref(height), ctypes.byref(format), ctypes.byref(buffer)
    ):

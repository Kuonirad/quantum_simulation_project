...
for name, function in [wrapPointerFunction(*args) for args in POINTER_FUNCTION_DATA]:
...
glVertexPointer = (
    wrapper.wrapper(_simple.glVertexPointer)
    .setPyConverter(
        "pointer",
        arrayhelpers.AsArrayOfType("pointer", "type"),
    )
    .setStoreValues(arrayhelpers.storePointerType("pointer", _simple.GL_VERTEX_ARRAY_POINTER))
    .setReturnValues(wrapper.returnPyArgument("pointer"))
)
glTexCoordPointer = (
    wrapper.wrapper(_simple.glTexCoordPointer)
    .setPyConverter(
        "pointer",
        arrayhelpers.AsArrayOfType("pointer", "type"),
    )
    .setStoreValues(arrayhelpers.storePointerType("pointer", _simple.GL_TEXTURE_COORD_ARRAY_POINTER))
    .setReturnValues(wrapper.returnPyArgument("pointer"))
)
glNormalPointer = (
    wrapper.wrapper(_simple.glNormalPointer)
    .setPyConverter(
        "pointer",
        arrayhelpers.AsArrayOfType("pointer", "type"),
    )
    .setStoreValues(arrayhelpers.storePointerType("pointer", _simple.GL_NORMAL_ARRAY_POINTER))
    .setReturnValues(wrapper.returnPyArgument("pointer"))
)
glIndexPointer = (
    wrapper.wrapper(_simple.glIndexPointer)
    .setPyConverter(
        "pointer",
        arrayhelpers.AsArrayOfType("pointer", "type"),
    )
    .setStoreValues(arrayhelpers.storePointerType("pointer", _simple.GL_INDEX_ARRAY_POINTER))
    .setReturnValues(wrapper.returnPyArgument("pointer"))
)
glEdgeFlagPointer = (
    wrapper.wrapper(_simple.glEdgeFlagPointer)
    .setPyConverter(
        # XXX type is wrong!
        "pointer",
        arrayhelpers.AsArrayTyped("pointer", arraydatatype.GLushortArray),
    )
    .setStoreValues(arrayhelpers.storePointerType("pointer", _simple.GL_EDGE_FLAG_ARRAY_POINTER))
    .setReturnValues(wrapper.returnPyArgument("pointer"))
)
glColorPointer = (
    wrapper.wrapper(_simple.glColorPointer)
    .setPyConverter(
        "pointer",
        arrayhelpers.AsArrayOfType("pointer", "type"),
    )
    .setStoreValues(arrayhelpers.storePointerType("pointer", _simple.GL_COLOR_ARRAY_POINTER))
    .setReturnValues(wrapper.returnPyArgument("pointer"))
)
glInterleavedArrays = (
    wrapper.wrapper(_simple.glInterleavedArrays)
    .setStoreValues(arrayhelpers.storePointerType("pointer", GL_INTERLEAVED_ARRAY_POINTER))
    .setReturnValues(wrapper.returnPyArgument("pointer"))
)


glDrawElements = (
    wrapper.wrapper(_simple.glDrawElements)
    .setPyConverter(
        "indices",
        arrayhelpers.AsArrayOfType("indices", "type"),
    )
...
def glDrawElementsTyped(type, suffix):
    ...
    function = (
        wrapper.wrapper(_simple.glDrawElements)
        .setPyConverter("type")
        .setCConverter("type", type)
        .setPyConverter("count")
        .setCConverter(
            "count",
            arrayhelpers.AsArrayTypedSize("indices", arrayType),
        )
        .setPyConverter(
            "indices",
            arrayhelpers.AsArrayTyped("indices", arrayType),
        )
        .setReturnValues(wrapper.returnPyArgument("indices"))
    )
    return function
...
for type, suffix in (
    (_simple.GL_UNSIGNED_BYTE, "ub"),
    (_simple.GL_UNSIGNED_INT, "ui"),
    (_simple.GL_UNSIGNED_SHORT, "us"),
):
...
def glRenderMode(newMode):
    ...
    if result < 0:
        if currentMode == _simple.GL_SELECT:
            ...
            raise error.GLError(
                _simple.GL_STACK_OVERFLOW,
                "Unknown glRenderMode buffer (%s) too small to hold selection results" % (currentMode,),
            )
    # Okay, now that the easy cases are out of the way...
    #  Do we have a pre-stored pointer about which the user already knows?
    context = platform.GetCurrentContext()
    if context == 0:
        raise error.Error("""Returning from glRenderMode without a valid context!""")

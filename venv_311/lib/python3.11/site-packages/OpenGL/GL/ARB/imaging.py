...
glColorTableParameterfv = wrapper.wrapper(glColorTableParameterfv).setInputArraySize("params", None)
# INPUT glColorTableParameteriv.params size not checked against 'pname'
glColorTableParameteriv = wrapper.wrapper(glColorTableParameteriv).setInputArraySize("params", None)
...
glColorSubTable = wrapper.wrapper(glColorSubTable).setInputArraySize("data", None)
# INPUT glConvolutionFilter1D.image size not checked against 'format,type,width'
glConvolutionFilter1D = wrapper.wrapper(glConvolutionFilter1D).setInputArraySize("image", None)
# INPUT glConvolutionFilter2D.image size not checked against 'format,type,width,height'
glConvolutionFilter2D = wrapper.wrapper(glConvolutionFilter2D).setInputArraySize("image", None)
# INPUT glConvolutionParameterfv.params size not checked against 'pname'
glConvolutionParameterfv = wrapper.wrapper(glConvolutionParameterfv).setInputArraySize("params", None)
# INPUT glConvolutionParameteriv.params size not checked against 'pname'
glConvolutionParameteriv = wrapper.wrapper(glConvolutionParameteriv).setInputArraySize("params", None)
...
glSeparableFilter2D = (
    wrapper.wrapper(glSeparableFilter2D).setInputArraySize("column", None).setInputArraySize("row", None)
)
...
def glGetConvolutionFilter(baseFunction, target, format, type):
    ...
    if target != GL_CONVOLUTION_1D:
        dims += (glGetConvolutionParameteriv(target, GL_CONVOLUTION_HEIGHT)[0],)
    ...
    arrayType = arrays.GL_CONSTANT_TO_ARRAY_TYPE[images.images.TYPE_TO_ARRAYTYPE.get(type, type)]
    baseFunction(target, format, type, ctypes.c_void_p(arrayType.dataPointer(array)))
    return array
...
def glGetHistogram(baseFunction, target, reset, format, type, values=None):
    ...
    arrayType = arrays.GL_CONSTANT_TO_ARRAY_TYPE[images.images.TYPE_TO_ARRAYTYPE.get(type, type)]
    baseFunction(target, reset, format, type, ctypes.c_void_p(arrayType.dataPointer(values)))
    return values

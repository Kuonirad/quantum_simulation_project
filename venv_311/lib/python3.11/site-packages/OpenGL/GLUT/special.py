...
if __glutInitWithExit:
    ...
    _base_glutInit = platform.nullFunction(
        ...
        doc="Initialize the GLUT library",
        argNames=("argcp", "argv"),
        module=__name__,
        error_checker=None,
    )
...
class GLUTCallback(object):
    ...
    def handler( %s ):
        return None""" % (
            typeName,
            ", ".join([describe(typ, name) for (typ, name) in zip(parameterTypes, parameterNames)]),
        )
        try:
        ...
        if GLUT_GUARD_CALLBACKS and hasattr(function, "__call__"):
            ...
            def safeCall(*args, **named):
                ...
                try:
                    ...
                    traceback.print_exc()
                    sys.stderr.write(
                        """GLUT %s callback %s with %s,%s failed: returning None %s\n"""
                        % (
                            self.typeName,
                            function,
                            args,
...
def glutInit(*args):
    ...
    if args:
        ...
        if isinstance(arg, integer_types):
            ...
            if count != len(args):
                raise ValueError(
                    """Specified count of %s does not match length (%s) of argument list %s"""
                    % (
                        count,
                        len(args),
                        args,

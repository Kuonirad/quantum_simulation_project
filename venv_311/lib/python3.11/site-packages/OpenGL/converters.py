...
class Converter(object):
    ...
    def finalise(self, wrapper):
        ...
        for indexname, argName, methodName in self.indexLookups:
            setattr(self, indexname, getattr(wrapper, methodName)(getattr(self, argName)))
...
if acceleratesupport.ACCELERATE_AVAILABLE:
    try:
        ...
        from OpenGL_accelerate.arraydatatype import Output, SizedOutput, OutputOrInput, SizedOutputOrInput
        from OpenGL_accelerate.wrapper import (
            returnCArgument,
            returnPyArgument,
        )
    except ImportError as err:
        _log.warning("Unable to load converters accelerators (wrapper, arraydatatype) from OpenGL_accelerate")
        CallFuncPyConverter = None
    ...
    class DefaultCConverter(CConverter):
        ...
        def __call__(self, pyArgs, index, wrapper):
            ...
            try:
                ...
                raise ValueError(
                    """Expected parameter index %r, but pyArgs only length %s""" % (self.index, len(pyArgs))
                )

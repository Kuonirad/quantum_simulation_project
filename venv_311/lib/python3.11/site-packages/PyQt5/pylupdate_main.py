...
from PyQt5.QtCore import PYQT_VERSION_STR, QDir, QFile, QFileInfo, QIODevice, QTextStream
            ...
            for key, value in proFileTagMap(fullText).items():
                for t in value.split(" "):
                    if key == "SOURCES":
                        ...
                        fetchtr_ui(_encoded_path(QDir.current().absoluteFilePath(t)), fetchedTor, defaultContext, True)

            updateTsFiles(fetchedTor, tsFileNames, codecForTr, noObsolete, verbose)

            if not metSomething:
                sys.stderr.write("pylupdate5 warning: File '%s' does not look like a " "project file\n" % arg)
            elif len(tsFileNames) == 0:
                sys.stderr.write("pylupdate5 warning: Met no 'TRANSLATIONS' entry in " "project file '%s'\n" % arg)
            ...
            if metTsFlag:
                if arg.lower().endswith(".ts"):
                    ...
                    if not fi.exists() or fi.isWritable():
                        ...
                        sys.stderr.write("pylupdate5 warning: For some reason, I " "cannot save '%s'\n" % arg)
                else:
                    sys.stderr.write("pylupdate5 error: File '%s' lacks .ts extension\n" % arg)
            else:
                fi = QFileInfo(arg)
                path = _encoded_path(fi.absoluteFilePath())

                if fi.suffix() in ("py", "pyw"):
                    fetchtr_py(path, fetchedTor, defaultContext, True, codecForSource, tr_func, translate_func)
                else:
